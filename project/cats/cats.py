...
def choose(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    p = [paragraph for paragraph in paragraphs if(select(paragraph))]
    if k < len(p):
        return p[k]
    else:
        return ''
...
def about(topic):
    ...
    def is_contain(paragraph):
        for t in topic:
            if t in [lower(s) for s in split(remove_punctuation(paragraph))]:
                return True
        return False
    return is_contain
    # END PROBLEM 2
...
def accuracy(typed, reference):
    ...
    if len(typed_words) == 0 and len(reference_words) == 0:
        ...
        while i < len(typed_words) and j < len(reference_words):
            ...
            i += 1
            j += 1
        return k / len(typed_words) * 100   #用了//,答案一直是0.0   100.0$$$$100
